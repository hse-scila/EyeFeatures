...
class DistanceTransformer(BaseTransformer):
    ...
    def get_expected_paths(self) -> dict[str, pd.DataFrame | np.ndarray]:
        return self.expected_paths

    def fit(self, X: pd.DataFrame, y=None):
        # check must-have attributes
        super()._check_init(self._get_required())
...
class SimpleDistances(DistanceTransformer):
    ...
    def get_feature_names_out(self, input_features=None) -> list[str]:
        ...
        for method in self.methods:
            cls_instance = self._methods_cls[method]
            feature_names.extend(cls_instance.get_feature_names_out())
        return feature_names

    def transform(self, X: Types.Data) -> pd.DataFrame | np.ndarray:
        # check must-have attributes
        super()._check_init(
            super()._get_required()
        ...
        data_part: Types.Partition = super()._get_partition(X)
...
class EucDist(DistanceTransformer):
    ...
    def transform(self, X: Types.Data) -> pd.DataFrame | np.ndarray:
        ...
        for group_nm, group_path in tqdm(data_part):
            ...
            path_group = super()._get_path_group(
...
class MultiMatchDist(DistanceTransformer):
    ...
    def get_feature_names_out(self, input_features=None) -> list[str]:
        return ["mm_shape", "mm_angle", "mm_len", "mm_pos", "mm_duration"]
