...
def scanpath_visualization(data_: pd.DataFrame, x: str, y: str, duration: str = None, dispersion: str = None,
                           time_stamps: str = None, img_path: str = None, fig_size: tuple = (10, 10),
                           points_width: float = 75, path_width: float = 2, points_color: str = None,
                           path_color: str = 'green', points_enumeration: bool = False, regression_color: str = None,
                           micro_sac_color: str = None, is_vectors: bool = False, min_dispersion: float = 1.2,
    ...
    if not is_vectors:
        ...
        plt.quiver(X.iloc[:-1], Y.iloc[:-1], X.shift(-1).iloc[:-1] - X.iloc[:-1],
                   Y.shift(-1).iloc[:-1] - Y.iloc[:-1], angles='xy', scale_units='xy', color=path_color, scale=1,
                   width=path_width / 500, edgecolor='yellow', linewidth=path_width / 1000)
        if regression_color is not None:
            regX = np.array([(X.iloc[i - 1], X.iloc[i]) for i in reg_only.index]).T
            regY = np.array([(Y.iloc[i - 1], Y.iloc[i]) for i in reg_only.index]).T
            plt.quiver(regX[0], regY[0], regX[1] - regX[0],
                       regY[1] - regY[0], angles='xy', scale_units='xy',
                       color=regression_color, scale=1, width=path_width / 500,
                       edgecolor='yellow', linewidth=path_width / 1000)

    if micro_sac_color is not None:     # TODO (doesn't work with normalized coordinates)
        assert dispersion is not None, "Error: provide 'dispersion' column before calling visualization"
        assert time_stamps is not None, "Error: provide 'time_stamps' column before calling visualization"
        assert duration is not None, "Error: provide 'duration' column before calling visualization"
        ...
        for i in range(len(mic_sacX)):
            plt.plot(mic_sacX[i], mic_sacY[i], color=micro_sac_color, linewidth=path_width)
