    ...
    def __init__(self, 
      ...
      for i in cnn_laeyrs_chanels:
        modules.append(nn.Sequential(
                      nn.Conv2d(in_chanels, i, kernel_size = kernel_size, stride = stride, padding = padding),
                      nn.BatchNorm2d(i),
                      nn.ReLU()))
        in_chanels = i
        padding=0
        shape = ((shape[0] - kernel_size + padding + stride)//stride, 
      ...
      self.accuracy = torchmetrics.Accuracy(task = 'binary' if n_classes==2 else 'multiclass')
